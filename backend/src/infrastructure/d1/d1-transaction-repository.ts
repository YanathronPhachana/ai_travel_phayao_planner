...
export class D1TransactionRepository implements TransactionRepository {
  ...
  async findByDateRange(startDate: string, endDate: string): Promise<Transaction[]> {
    const { results } = await this.db
      .prepare('SELECT id, type, amount, category, description, date, created_at FROM transactions WHERE date >= ? AND date <= ? ORDER BY date DESC, created_at DESC')
  ...
  async findByType(type: 'income' | 'expense'): Promise<Transaction[]> {
    const { results } = await this.db
      .prepare('SELECT id, type, amount, category, description, date, created_at FROM transactions WHERE type = ? ORDER BY date DESC, created_at DESC')
      .bind(type)
      .all<TransactionRow>()
    return results.map(toTransaction)
  }
  ...
  async create(input: CreateTransactionInput): Promise<Transaction> {
    ...
    await this.db
      ...
      .bind(id, input.type, input.amount, input.category, description, input.date, createdAt)
      .run()
    return { id, type: input.type, amount: input.amount, category: input.category, description, date: input.date, createdAt }
  }
